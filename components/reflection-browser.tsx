...
export function ReflectionBrowser({ entries }: { entries: ReflectionEntry[] }) {
  ...
  return <div className="reflection-browser">
    ...
    {!filtered.length && <div className="reflection-empty"><h2>{entries.length === 0 ? 'Belum ada data.' : 'Belum ada yang cocok.'}</h2>{entries.length > 0 && <p>Coba kata kunci, topik, atau penulis lain.</p>}{hasFilters && <button className="button button-dark" type="button" onClick={clearFilters}>Tampilkan semua <span aria-hidden="true">↗</span></button>}</div>}
  </div>
}
